/**
 * JavaCoursePage — Course index dashboard for Advanced Java
 * Lists all 15 modules as interactive glassmorphism cards with progress indicators, XP, and badges
 * Uses global ThemeContext for consistent dark/light theming
 */
...
const DIFFICULTY_COLORS = {
  ...
  Expert:       { light: { bg: '#f3e8ff', text: '#9333ea', border: '#c4b5fd' }, dark: { bg: 'rgba(139,92,246,0.15)', text: '#a78bfa', border: 'rgba(139,92,246,0.3)' } },
};

const BADGES_INFO = [
  { id: 'jdbc-master', name: 'JDBC Master', icon: '🔌', desc: 'Completed JDBC Fundamentals' },
  { id: 'servlet-expert', name: 'Servlet Expert', icon: '⚙️', desc: 'Completed Servlets' },
  { id: 'hibernate-hero', name: 'Hibernate Hero', icon: '❄️', desc: 'Completed Hibernate' },
  { id: 'spring-ninja', name: 'Spring Ninja', icon: '🌱', desc: 'Completed Spring Core' },
  { id: 'security-guardian', name: 'Security Guardian', icon: '🛡️', desc: 'Completed Spring Security' },
  { id: 'microservices-architect', name: 'Microservices Architect', icon: '🌐', desc: 'Completed Microservices' },
  { id: 'enterprise-java-engineer', name: 'Enterprise Java Engineer', icon: '🏆', desc: 'Completed Enterprise Project' },
];

/* ── Module Card Component ── */
function JavaLessonCard({ lesson, index, navigate, isDark }) {
  const isCompleted = useJavaStore(s => s.isCompleted(lesson.slug));
  const progress    = useJavaStore(s => s.getProgress(lesson.slug));
  const quizScore   = useJavaStore(s => s.getQuizScore(lesson.slug));
  const dc = DIFFICULTY_COLORS[lesson.difficulty] || DIFFICULTY_COLORS.Intermediate;
  const colors = isDark ? dc.dark : dc.light;

  const handleClick = () => navigate(`/advanced-java/course/${lesson.slug}`);

  // Glassmorphism card styles based on theme
...
export default function JavaCoursePage() {
  ...
  const completed  = useJavaStore(s => s.completedLessons);
  const badges     = useJavaStore(s => s.badges);
  const overall    = useJavaStore(s => s.getOverallProgress(JAVA_LESSONS));

  const totalXP    = useMemo(() => JAVA_LESSONS.reduce((s, l) => s + l.xp, 0), []);
  const totalMins  = useMemo(() => JAVA_LESSONS.reduce((s, l) => s + parseInt(l.duration), 0), []);
  ...
  return (
    <div className="space-y-8">
      ...
      <motion.div
        ...
        style={{
          background: isDark
            ? 'linear-gradient(135deg, #100b26 0%, #0d1b2a 40%, #05162a 100%)'
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-bold uppercase tracking-widest text-orange-300 bg-orange-500/15 border border-orange-400/25 rounded-full px-2.5 py-0.5">
                AI Architect Academy
              </span>
            </div>
            <h1 className="text-3xl font-black text-white mb-2">Advanced Enterprise Java</h1>
            <p className="text-slate-300 text-sm max-w-xl leading-relaxed">
              Master JDBC architecture, Servlets, Hibernate ORM mappings, Spring Core DI container lifecycle, 
              API gateways, and secure stateless JWT microservices. Designed like an elite coding academy.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 flex-shrink-0 w-full md:w-auto">
            {[
              { icon: BookOpen,   label: 'Modules',    value: JAVA_LESSONS.length, color: '#60a5fa' },
      ...
      >
        <div className="flex items-center gap-3 w-full md:w-auto flex-1">
          ...
          <div className="flex-1">
            <div className="flex justify-between text-sm mb-1.5">
              ...
              <span className={`font-bold ${isDark ? 'text-amber-400' : 'text-amber-600'}`}>{xpEarned} XP</span>
            </div>
        ...
        {overall === 100 && (
          <button
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-bold rounded-xl transition duration-300 transform hover:scale-105 active:scale-95 shadow-md shadow-amber-500/20 whitespace-nowrap text-sm flex items-center gap-2"
          >
            🎓 Claim Enterprise Java Certificate
          </button>
        )}
      </motion.div>
      ...
      >
        <h2 className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-4 flex items-center gap-2`}>
          <Trophy size={16} className={isDark ? 'text-amber-400' : 'text-amber-500'} /> Badges Collection
        </h2>
          ...
          {BADGES_INFO.map(badge => {
            ...
            return (
              ...
              >
                ...
                <div className={`text-[8px] ${isDark ? 'text-slate-500' : 'text-slate-500'} leading-normal mt-0.5`}>{badge.desc}</div>
              </div>
            );
          })}
        </div>
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          ...
          >
            {JAVA_LESSONS.length} modules
          </span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {JAVA_LESSONS.map((lesson, index) => (
            <JavaLessonCard key={lesson.id} lesson={lesson} index={index} navigate={navigate} isDark={isDark} />
          ))}
        </div>
      </div>
