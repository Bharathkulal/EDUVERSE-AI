...
const load = () => {
  try {
    const raw = localStorage.getItem('eduverse_core_java_progress');
...
const save = (state) => {
  try {
    localStorage.setItem('eduverse_core_java_progress', JSON.stringify(state));
  } catch { /* ignore */ }
};

const useCoreJavaStore = create((set, get) => {
  const saved = load();
  return {
    ...
    completeLesson: (slug, xp) => {
      ...
      if (slug === 'java-basics' && !newBadges.includes('basics-master')) {
        newBadges.push('basics-master');
      } else if (slug === 'variables-datatypes' && !newBadges.includes('variables-master')) {
        newBadges.push('variables-master');
      } else if (slug === 'operators-operations' && !newBadges.includes('operators-master')) {
        newBadges.push('operators-master');
      } else if (slug === 'oop-concepts' && !newBadges.includes('oop-master')) {
        newBadges.push('oop-master');
      } else if (slug === 'collections-framework' && !newBadges.includes('collections-master')) {
        newBadges.push('collections-master');
      }
    ...
    resetAll: () => {
      localStorage.removeItem('eduverse_core_java_progress');
      set({
        completedLessons: [],
        lessonProgress:   {},
        quizScores:       {},
...
export default useCoreJavaStore;
