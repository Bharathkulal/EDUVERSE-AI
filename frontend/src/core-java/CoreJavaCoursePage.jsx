...
const BADGES_INFO = [
  { id: 'basics-master', name: 'Basics Master', icon: '☕', desc: 'Completed Java Basics & Architecture' },
  { id: 'variables-master', name: 'Variables Master', icon: '📦', desc: 'Completed Variables & Data Types' },
  { id: 'operators-master', name: 'Operators Master', icon: '⚡', desc: 'Completed Operations & Operators' },
  { id: 'oop-master', name: 'OOP Master', icon: '🧩', desc: 'Completed Object-Oriented Programming' },
  { id: 'collections-master', name: 'Collections Master', icon: '📚', desc: 'Completed Collections Framework' }
];

function JavaLessonCard({ lesson, index, navigate, isDark }) {
  const isCompleted = useCoreJavaStore(s => s.isCompleted(lesson.slug));
  const progress    = useCoreJavaStore(s => s.getProgress(lesson.slug));
  const quizScore   = useCoreJavaStore(s => s.getQuizScore(lesson.slug));
  const dc = DIFFICULTY_COLORS[lesson.difficulty] || DIFFICULTY_COLORS.Intermediate;
  const colors = isDark ? dc.dark : dc.light;

  const handleClick = () => navigate(`/core-java/course/${lesson.slug}`);
...
export default function CoreJavaCoursePage() {
  ...
  const overall    = useCoreJavaStore(s => s.getOverallProgress(CORE_JAVA_LESSONS));

  const totalXP    = useMemo(() => CORE_JAVA_LESSONS.reduce((s, l) => s + l.xp, 0), []);
  const totalMins  = useMemo(() => CORE_JAVA_LESSONS.reduce((s, l) => s + parseInt(l.duration), 0), []);
    ...
    <div className="space-y-8">
      <motion.div
        ...
        style={{
          ...
          border: isDark ? '1px solid rgba(245,158,11,0.3)' : '1px solid rgba(245,158,11,0.4)',
          boxShadow: isDark
      ...
      >
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          <motion.div
            ...
            style={{ background: 'radial-gradient(circle, #F59E0B, transparent)', top: '-100px', right: '-50px' }}
            animate={{ scale: [1, 1.2, 1], opacity: [0.1, 0.15, 0.1] }}
            transition={{ duration: 5, repeat: Infinity }}
          />
          <motion.div
            className="absolute w-64 h-64 rounded-full opacity-10"
            style={{ background: 'radial-gradient(circle, #EF4444, transparent)', bottom: '-50px', left: '20%' }}
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-6">
          ...
          <div className="flex-1">
            ...
            <h1 className="text-3xl font-black text-white mb-2">Core Java Foundations</h1>
            <p className="text-slate-300 text-sm max-w-xl leading-relaxed">
              Master Java syntax, compiler details, operations logic, OOP architecture, exceptions handling, 
              and Java Collections. Designed for aspiring software engineers.
            </p>
          </div>

          <div className="grid grid-cols-2 gap-3 flex-shrink-0 w-full md:w-auto">
            {[
              { icon: BookOpen,   label: 'Modules',    value: CORE_JAVA_LESSONS.length, color: '#60a5fa' },
      ...
      >
        <div className="flex items-center gap-3 w-full md:w-auto flex-1">
          ...
          >
            <Star size={18} className={isDark ? 'text-orange-400' : 'text-orange-500'} />
          </div>
          <div className="flex-1">
            <div className="flex justify-between text-sm mb-1.5">
              <span className={`font-semibold ${isDark ? 'text-slate-200' : 'text-slate-800'}`}>Academy XP Score</span>
              <span className={`font-bold ${isDark ? 'text-orange-400' : 'text-orange-600'}`}>{xpEarned} XP</span>
            </div>
        ...
        {overall === 100 && (
          <button
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-orange-500 to-orange-600 hover:from-orange-600 hover:to-orange-700 text-white font-bold rounded-xl transition duration-300 transform hover:scale-105 active:scale-95 shadow-md shadow-orange-500/20 whitespace-nowrap text-sm flex items-center gap-2"
          >
            🎓 Claim Core Java Certificate
          </button>
        )}
      </motion.div>
      ...
      >
        <h2 className={`text-base font-bold ${isDark ? 'text-white' : 'text-slate-900'} mb-4 flex items-center gap-2`}>
          <Trophy size={16} className={isDark ? 'text-orange-400' : 'text-orange-555'} /> Badges Collection
        </h2>
      ...
      <div>
        <div className="flex items-center gap-3 mb-5">
          <Code2 size={18} className={isDark ? 'text-orange-400' : 'text-orange-600'} />
          ...
          >
            {CORE_JAVA_LESSONS.length} modules
          </span>
        </div>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {CORE_JAVA_LESSONS.map((lesson, index) => (
